...
# MCP imports (optional - only loaded if MCP mode is enabled)
try:
    ...
    MCP_AVAILABLE = False
    logger.warning("MCPモジュールが見つかりません。MCP機能は無効です。")
...
class MCPServerHandler:
    ...
    async def _get_model_list(self) -> dict:
        """モデル一覧を取得"""
        result = await model_command("list", "", "mcp")
        return result.get("data", {})

    async def _get_model_info(self, model_name: str) -> dict:
        """モデル情報を取得"""
        expressions = await model_command("get_expressions", model_name, "mcp")
        motions = await model_command("get_motions", model_name, "mcp")
        parameters = await model_command("get_parameters", model_name, "mcp")
    ...
    async def _set_expression(self, client_id: str, expression: str) -> dict:
        """表情を設定"""
        return await client_command("set_expression", expression, client_id, "mcp")

    async def _set_motion(self, client_id: str, group: str, no: int, priority: int = 2) -> dict:
        """モーションを設定"""
        args = f"{group} {no} {priority}"
        return await client_command("set_motion", args, client_id, "mcp")

    async def _set_parameter(self, client_id: str, parameters: dict) -> dict:
        """パラメータを設定"""
        return await client_command("set_parameter", parameters, client_id, "mcp")

    async def _list_clients(self) -> dict:
        """クライアント一覧を取得"""
        result = await process_command("list", "mcp")
        return result.get("data", {})

    async def _get_client_state(self, client_id: str) -> dict:
        """クライアントの状態を取得"""
        model = await client_command("get_model", "", client_id, "mcp")
        expression = await client_command("get_expression", "", client_id, "mcp")
        motion = await client_command("get_motion", "", client_id, "mcp")
        eye_blink = await client_command("get_eye_blink", "", client_id, "mcp")
        breath = await client_command("get_breath", "", client_id, "mcp")
    ...
    async def _set_eye_blink(self, client_id: str, enabled: bool) -> dict:
        ...
        return await client_command("set_eye_blink", state, client_id, "mcp")

    async def _set_breath(self, client_id: str, enabled: bool) -> dict:
        """呼吸を設定"""
        state = "enabled" if enabled else "disabled"
        return await client_command("set_breath", state, client_id, "mcp")
...
async def run():
    ...
    # MCPモードチェック
    if args.mode in ['mcp', 'both'] and not MCP_AVAILABLE:
